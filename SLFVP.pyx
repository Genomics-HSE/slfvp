...
cdef class Individual:
    ...
    def __init__(self, Py_ssize_t p_id, Py_ssize_t i_id, double time, double x, double y, Py_ssize_t[:] i_type):
        self.p_id = p_id
        self.i_id = i_id
        self.time = time
        self.x = x
...
cdef class State:
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef void remove(self, Py_ssize_t i_id):
        self.n_alive -= 1
        self.n_dead += 1
...
cdef inline double tor2_distance(double x1, double x2, double y1, double y2,  double w=1, double h=1):
    return (min(abs(x1 - y1), w - abs(x1 - y1))**2 + min(abs(x2 - y2), h - abs(x2 - y2))**2)**0.5
...
cdef class SLFVP:
    ...
    @cython.wraparound(False)
    @cython.cdivision(True)
    cpdef void extinction(self, double z1, double z2):
        cdef double x1, x2
        cdef Py_ssize_t i
        cdef list ids
        cdef list list_alive = list(self.state.ids_alive)
